...
3. Report the **retrieval plane and capabilities**: run `codastre version`. v0.18.0+ → the cheap `agent` rung works on both planes; v0.14.0–v0.17.x → CLI plane only (MCP `agent` is swallowed by this client); older → MCP `verbose` only. Then one line listing what the binary unlocks and what it's missing, from: `corpora` (v0.15.0), `contracts` (v0.17.0), `--stacks` (v0.18.1), `--client` attribution (v0.19.0). Mention updating only when something is missing (v0.19.1 is current). Don't guess an install channel.
...
| `codastre version` v0.14.0–v0.17.x | MCP `format: "agent"` returns no results in Claude Code — use the CLI plane; v0.18.0+ fixes it |
| `CORPUS_SEARCH`/`CONTRACTS` tools missing | Server predates them, or Claude Code needs a restart to refresh the tool list |
