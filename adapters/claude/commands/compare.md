...
**Which plane and rung Agent A runs at — decided here, not by the subagent.** On a v0.14.0–v0.17.x binary, never tell Agent A to "try `format: "agent"`" over MCP (from v0.18.0 it works — see below). On those binaries, in `agent` format the payload is in `content[0].text` and `structuredContent` is a fixed summary, and Claude Code prefers `structuredContent` when both are present, so the model gets the summary alone — verified 2026-08-18 (`codastre` v0.14.0). A subagent that tries it burns a call for nothing and then retries at `verbose`, which lands in the token log as a real cost with no answer attached. Three of four earlier runs on one corpus did exactly that, and it is why their Codastre totals are uninterpretable (`core/measurement.md`, "Observation (2026-08-18)").
